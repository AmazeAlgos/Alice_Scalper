...
class MiniScalper(QtWidgets.QMainWindow, Ui_MainWindow,Ui_SecondWindow,Ui_ThirdWindow):
    ...
    def create_login_window(self):
        ...
        if self.login_window is None:
            ...
            self.ui_second_window.btn_login.clicked.connect(self.BrokerLogin)
            self.ui_second_window.btn_logout.clicked.connect(self.BrokerLogout)
    ...
    def place_order(self):
        ...
        ret = api.place_order(transaction_type = TransactionType.Buy,
    ...
    def exit_order(self):
        global api,ord_qty,trading_symbol,ord_no,ord_stat
        ret = api.place_order(transaction_type = TransactionType.Sell,
                                        instrument = trading_symbol,
                                        quantity = ord_qty,
                                        order_type = OrderType.Market,
                                        product_type = ProductType.Intraday,
    ...
    def order_book(self):
        ...
        self.ord_data = api.order_data()
        #print(self.ord_data)
        #sys.exit()
        try:
            if self.ord_data is not None:
                ord_df = pd.DataFrame(self.ord_data)
                myord_df = ord_df[["Trsym", "Qty", "Trantype", "Prc", "Status", "Prctype"]].copy()
                #myord_df['Trsym'] = myord_df['Trsym'].apply(lambda x: re.search("^(.)(\D+)(\d+)(\D+)(\d+)(\D+)(\d+)", x).group(1) + re.search("^(.)(\D+)(\d+)(\D+)(\d+)(\D+)(\d+)", x).group(7) + re.search("^(.)(\D+)(\d+)(\D+)(\d+)(\D+)(\d+)", x).group(6) if re.search("^(.)(\D+)(\d+)(\D+)(\d+)(\D+)(\d+)", x) is not None else '')
                myord_df.rename(columns={'Trsym': 'Strike', 'Qty': 'Qty', 'Trantype': 'Ord', 'Prc': 'Prc', 'Status': 'Status', 'Prctype': 'Prd. Typ.'}, inplace=True)
    ...
    def update_var_token(self):
        ...
        expiry1 = datetime.strptime(expiry, '%d-%b-%Y').date()
        #trading_symbol=f"{ticker}{expiry_date}{optype[0]}{mystrike}"

        trading_symbol = api.get_instrument_for_fno(exch='NFO',symbol = ticker, expiry_date=expiry1.isoformat(), is_fut=False, strike=mystrike, is_CE = (optype == 'CE'))
        ...
        if float(mystrike) > 10000 :
            self.var_token = int(trading_symbol[1])
            self.lbl_strike.setText(str(mystrike))
            #print(self.var_token)
            api.subscribe([trading_symbol])
    ...
    def liveprice(self):
        ...
        banknifty='26009'
        nifty='26000'
        bnftoken=api.get_instrument_by_symbol("INDICES", "Nifty Bank")
        nftoken=api.get_instrument_by_symbol("INDICES", "Nifty 50")



        def subscribe_ins():
            api.subscribe([bnftoken,nftoken])


        def event_handler_feed_update(message):
            feed_message = json.loads(message)
            # print(feed_message)
            if feed_message["t"]=='tf' or feed_message["t"]=='tk':
                # print(feed_message['tk'],feed_message['lp'])
                # print(('lp' in feed_message),('tk' in feed_message))
                if (('lp' in feed_message)&('tk' in feed_message)):
                    feedJson[feed_message['tk']]={'ltp':float(feed_message['lp'])}


        # def event_handler_order_update(inmessage):
        #     if (('NOrdNo' in inmessage)&('status' in inmessage)):
        #         orderJson[inmessage['NOrdNo']]={'status':float(inmessage['status'])} 


        def error_callback(error):
            print(" In error_callback(). {}".format(error))
        ...
        def setup_websocket():
            ...
            api.start_websocket(socket_open_callback=open_callback,socket_error_callback=error_callback,
                                subscription_callback=event_handler_feed_update, run_in_background=True)
    ...
    def BrokerLogin(self):
        ...
        with open('cred.yml') as f:
            ...
            #print(cred)
        api = Aliceblue(user_id=cred['user'],api_key=cred['apikey'])
        ret = api.get_session_id() # Get Session ID
        ...
        if ret['stat'] == 'Ok' :
            self.ui_second_window.username.setText(api.get_profile()['accountId'])
